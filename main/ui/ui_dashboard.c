...
// Helpers
static void format_time_str(char *buf, size_t buf_size) {
    time_t now;
    struct tm timeinfo;
    time(&now);
    localtime_r(&now, &timeinfo);
    snprintf(buf, buf_size, "%02d:%02d:%02d", timeinfo.tm_hour, timeinfo.tm_min, timeinfo.tm_sec);
}
...
static void create_top_bar(lv_obj_t *parent) {
    ...
    lv_label_set_text(s_device_name_label, "AgentRing-ESP32");
    ...
    lv_obj_set_style_text_font(s_updated_label, &lv_font_montserrat_14, LV_PART_MAIN);
...
static void build_provider_column(lv_obj_t *parent, const provider_data_t *provider, int total_count) {
    ...
    int ring_size = (total_count <= 2) ? 220 : (total_count == 3 ? 190 : 165);
    ...
    lv_obj_set_style_text_font(pct_label, (ring_size >= 200) ? &lv_font_montserrat_36 : &lv_font_montserrat_28, LV_PART_MAIN);
    ...
    for (int r = 0; r < provider->row_count; r++) {
        ...
        lv_obj_set_style_text_font(r_lbl, &lv_font_montserrat_14, LV_PART_MAIN);
        ...
        if (row->reset[0] != '\0') {
            ...
            lv_obj_set_style_text_font(rst_lbl, &lv_font_montserrat_14, LV_PART_MAIN);
...
void ui_dashboard_update_payload(const sync_payload_t *payload) {
    ...
    // Update timestamp
    char time_buf[32];
    format_time_str(time_buf, sizeof(time_buf));
    char updated_str[48];
    snprintf(updated_str, sizeof(updated_str), "最后更新 %s", time_buf);
